...
let motivosGlobais = [];
let imagemPadraoPastas = ""; // Nova variável para a logo padrão
...
function renderizarPastasGenericas(colecao) {
    ...
    if (dadosAtuais.length === 0) {
        grid.innerHTML = '<p style="color: var(--text-muted); font-size: 14px;">Nenhuma pasta encontrada ou dados carregando...</p>';
        return;
    }
    ...
    pastasUnicas.forEach(nomePasta => {
        ...
        // Pega a cor do primeiro item. Se não tiver, usa a cor principal
        ...
        // Lógica da Imagem Padrão (Se tiver imagem no ajuste, mostra a imagem, senão o ícone)
...
// ================= GERAÇÃO DOS CARDS E RENDERIZAÇÃO =================
function gerarHTMLCard(colecaoNome, docId, data) {
    ...
    // Removendo has-gradient problemático dos contatos para não bugar a cor da fonte
        ...
        if (colecaoNome === 'ramais') {
            ...
            Object.keys(locaisMap).sort().forEach(local => {
                ...
                locaisMap[local].sort((a,b) => (a.data['Setor']||'').localeCompare(b.data['Setor']||'')).forEach(item => {
                    ...
                    // Força fundo branco para não estragar a cor da fonte no ramal
                    const corSalva = data.corCard && data.corCard !== "transparent" ? data.corCard : "#ffffff";
...
// ================= LÓGICA DE ABERTURA E FECHAMENTO DE MODAL =================
window.fecharModal = function() {
    const modalEl = document.getElementById('modal-cadastro');
    if(modalEl) modalEl.style.display = 'none';
}

// Vincula o botão de fechar (X)
document.getElementById('btn-fechar-modal').addEventListener('click', window.fecharModal);
...
// LÓGICA DE SALVAR CONFIGURAÇÕES (AJUSTES)
document.getElementById('btn-salvar-ajustes').addEventListener('click', async () => {
    ...
    document.getElementById('btn-salvar-ajustes').innerHTML = "<i class='ri-loader-4-line ri-spin'></i> Salvando...";
    try {
